...
from game_data import (
    ...
    RANDOM_EVENTS, MUSIC_GAMES, NOTIFICATION_REASON_TEXT, STATE_IMAGE_ALIASES, EGG_IMAGE_KEYS,
)
...
def is_adult_like(row): return row["stage"] == "adult" or row["character_id"].startswith("adult_") or row["character_id"].startswith("secret_")
def poop_enabled(row): return (not is_egg(row)) and (not is_adult_like(row))
...
def notification_mode_label(mode: str):
    return {"tamagotchi": "たまごっち", "normal": "ふつう", "quiet": "静か", "mute": "ミュート"}.get(mode, "たまごっち")
...
def sleep_window_label(row) -> str:
    ...
    return f"{setting['sleep_start']}〜{setting['sleep_end']}"
...
def asset_bases_for_row(row):
    info = CHARACTERS.get(row["character_id"], {})
    candidates = [info.get("asset_base"), info.get("name"), row.get("character_id", "")]
    candidates.extend(info.get("asset_aliases", []) or [])
    out = []
...
def state_key_names(state: str):
    names = []
    for item in STATE_IMAGE_ALIASES.get(state, []):
...
def image_keys_for_pet(row, transient=None):
    if row["character_id"] == "egg_yuiran":
        keys = list(EGG_IMAGE_KEYS)
        if transient == "hatch":
            return ["卵割れる", "たまご割れる", *keys]
        return keys
    if transient == "feed":
        state = "feed"
        ...
        state = "normal"
    keys = []
    for base in asset_bases_for_row(row):
        for suffix in state_key_names(state):
            key = f"{base}_{suffix}"
            if key not in keys:
                keys.append(key)
    if state != "normal":
        for base in asset_bases_for_row(row):
            for suffix in state_key_names("normal"):
                key = f"{base}_{suffix}"
                if key not in keys:
                    keys.append(key)
    return keys
...
def image_keys_for_debug(row):
    return image_keys_for_pet(row)
...
def status_lines(row):
    ...
    lines += [f"たいちょう {health}", f"いま　 {sleeping}", f"状態 {call}{whim}"]
    if row["praise_pending"]:
        lines.append("✨ ほめてサイン")
    if row.get("good_behavior_pending", 0):
        lines.append("🙏 いいことサイン")
    if is_egg(row):
...
def build_check_text(row):
    lines = [
        ...
        f"時計補正：{clock_offset_label(row)}",
        f"ねる時間：{sleep_window_label(row)}",
        "",
        f"おなか　 {bar(row['hunger'])}",
        f"ごきげん {bar(row['mood'])}",
        f"しつけ　 {row['discipline']}",
    ...
    lines += [
        ...
        f"状態：{'ねている' if row['is_sleeping'] else 'おきている'}",
        f"音：{sound_label(row)}",
...
def get_decay_profile(row):
    ...
    if stage == "adult": return {"hunger_minutes": 26, "mood_minutes": 30, "sleep_gain_minutes": 60, "poop_minutes": 999999}
    return {"hunger_minutes": 9999, "mood_minutes": 9999, "sleep_gain_minutes": 9999, "poop_minutes": 999999}
...
def evolve_if_needed(user_id, row):
    ...
    if cid in STAGE_SECONDS and now - row["stage_entered_at"] >= STAGE_SECONDS[cid]:
        ...
        if next_stage == "adult":
            updates.update({"poop": 0})
            if row.get("call_reason") == "poop":
                updates.update({"call_flag": 0, "call_reason": None})
...
def whim_check(row, now):
    ...
    essentials_ok = row["hunger"] >= 3 and row["mood"] >= 3 and row["is_sick"] == 0 and row["sleepiness"] < 80 and (row["poop"] == 0 or not poop_enabled(row))
...
def update_over_time(user_id, row):
    ...
    temp = dict(row)
    temp.update({"hunger": hunger, "mood": mood, "sleepiness": sleepiness, "poop": poop, "is_sick": is_sick, "is_whim_call": is_whim_call, "is_sleeping": is_sleeping})
    ...
    local_dt = ts_to_local(now, user_id=user_id, row=row)
    ...
    database.update_pet(
        user_id,
        hunger=hunger, mood=mood, sleepiness=sleepiness, affection=affection, stress=stress, poop=poop,
        is_sick=is_sick, sickness_count=sickness_count,
        call_flag=call_flag, call_reason=call_reason if call_reason else whim_reason, call_started_at=call_started_at, call_stage=call_stage,
        is_whim_call=is_whim_call, is_sleeping=is_sleeping, lights_off=lights_off, weight=weight,
        praise_pending=praise_pending, praise_due_at=praise_due_at, good_behavior_pending=good_behavior_pending, good_behavior_due_at=good_behavior_due_at,
        last_whim_at=last_whim_at, age_seconds=row["age_seconds"] + diff, last_access_at=now,
        care_miss_count=care_miss_count, night_visit_count=night_visit_count, evolution_warned=evo_warned,
    )
    row = database.fetch_pet(user_id)
    if row and not poop_enabled(row) and (row.get("poop", 0) != 0 or row.get("call_reason") == "poop"):
        database.update_pet(
            user_id,
            poop=0,
            call_flag=0 if row.get("call_reason") == "poop" else row.get("call_flag", 0),
...
def perform_action(user_id, row, action):
    ...
    if action == "feed":
        ...
        database.update_pet(user_id, hunger=clamp_meter(row["hunger"] + 2), mood=clamp_meter(row["mood"] + bonus.get("mood", 0)), total_feed_count=row["total_feed_count"] + 1, affection=clamp(row["affection"] + 2), weight=min(30, row["weight"] + 1), call_flag=0 if row["call_reason"] == "hunger" else row["call_flag"], call_reason=None if row["call_reason"] == "hunger" else row["call_reason"], last_access_at=now)
        result = "🍚 ごはんをたべた！"
        transient = "feed"
    elif action == "snack":
        bonus = personality_bonus(row["character_id"], "snack")
        database.update_pet(user_id, mood=clamp_meter(row["mood"] + 1 + bonus.get("mood", 0)), stress=clamp(row["stress"] + 4), total_snack_count=row["total_snack_count"] + 1, affection=clamp(row["affection"] + 1), weight=min(30, row["weight"] + 1), last_access_at=now)
        result = "🍰 おやつをたべてうれしそう！"
        transient = "snack"
    elif action == "play":
        bonus = personality_bonus(row["character_id"], "play")
        database.update_pet(user_id, mood=clamp_meter(row["mood"] + 2 + bonus.get("mood", 0)), stress=clamp(row["stress"] - 15), total_play_count=row["total_play_count"] + 1, affection=clamp(row["affection"] + 3 + bonus.get("affection", 0)), call_flag=0 if row["call_reason"] == "mood" else row["call_flag"], call_reason=None if row["call_reason"] == "mood" else row["call_reason"], last_access_at=now)
        result = "🎵 たのしくあそんだ！"
    elif action == "sleep":
        if row["sleepiness"] < 65:
            ...
            database.update_pet(user_id, is_sleeping=1, lights_off=1, sleepiness=clamp(row["sleepiness"] - 20 + bonus.get("sleepiness", 0)), total_sleep_count=row["total_sleep_count"] + 1, call_flag=0 if row["call_reason"] == "sleepy" else row["call_flag"], call_reason=None if row["call_reason"] == "sleepy" else row["call_reason"], last_access_at=now)
        ...
        if row["is_whim_call"]:
            ...
            database.update_pet(user_id, discipline=row["discipline"] + 1 + bonus.get("discipline", 0), mood=clamp_meter(row["mood"] - 1 if row["mood"] > 0 else 0), is_whim_call=0, call_flag=0, call_reason=None, total_discipline_count=row["total_discipline_count"] + 1, last_access_at=now)
        ...
        if row["praise_pending"] or row.get("good_behavior_pending", 0):
            database.update_pet(user_id, praise_pending=0, praise_due_at=0, good_behavior_pending=0, good_behavior_due_at=0, affection=clamp(row["affection"] + 6), mood=clamp_meter(row["mood"] + 1), stress=clamp(row["stress"] - 6), total_praise_count=row.get("total_praise_count", 0) + 1, last_access_at=now)
            result = "✨ ほめた！ うれしそう！"
        else:
            database.update_pet(user_id, affection=clamp(row["affection"] - 1), stress=clamp(row["stress"] + 2), last_access_at=now)
        ...
        if not poop_enabled(row):
            ...
            database.update_pet(user_id, poop=0, stress=clamp(row["stress"] - 12 + bonus.get("stress", 0)), total_clean_count=row["total_clean_count"] + 1, call_flag=0 if row["call_reason"] == "poop" else row["call_flag"], call_reason=None if row["call_reason"] == "poop" else row["call_reason"], last_access_at=now)
            result = "🧹 うんちをきれいにした！"
    elif action == "medicine":
        if row["is_sick"]:
            cured = 1 if random.randint(1, 100) <= 85 else 0
            database.update_pet(user_id, is_sick=0 if cured else 1, stress=clamp(row["stress"] - 8), total_medicine_count=row["total_medicine_count"] + 1, call_flag=0 if cured and row["call_reason"] == "sick" else row["call_flag"], call_reason=None if cured and row["call_reason"] == "sick" else row["call_reason"], last_access_at=now)
...
def resolve_minigame(user_id, row, game_key, choice_index):
    ...
    database.update_pet(user_id, **updates)
    row = database.fetch_pet(user_id)
    evo = evolve_if_needed(user_id, row)
    return database.fetch_pet(user_id), msg, evo
...
def call_message_text(user_mention: str, row):
    ...
    stage = int(row.get("call_stage", 1) or 1)
    if stage <= 1:
        head, lead = "● 注意アイコン点灯中", "おせわサイン"
    elif stage == 2:
        head, lead = "●● 注意アイコン点灯中", "まだおせわできてないよ！"
    else:
        head, lead = "●●● 注意アイコン点灯中", "かなりこまってるよ！"
    return f"{head}\n{beep}{lead}\n{user_mention}\n{title}\n{body}\n👉 おすすめ：{recommend}"


def set_display_clock_to_hhmm(user_id: int, hhmm: str):
    target = normalize_hhmm(hhmm)
    now = datetime.now(BASE_TIMEZONE)
    h, m = parse_hhmm(target)
    desired = now.replace(hour=h, minute=m, second=0, microsecond=0)
    delta_minutes = round((desired - now).total_seconds() / 60)
    database.set_clock_offset_minutes(user_id, delta_minutes)


def shift_clock_minutes(user_id: int, delta_minutes: int):
    current = _clock_offset_minutes_from_setting(user_id=user_id)
    database.set_clock_offset_minutes(user_id, current + int(delta_minutes))


def reset_clock_to_jst(user_id: int):
    database.set_clock_offset_minutes(user_id, 0)
