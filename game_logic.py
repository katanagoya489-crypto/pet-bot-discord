...
try:
    from config import SLEEP_END, SLEEP_START
except Exception:
    SLEEP_START = "22:00"
    SLEEP_END = "07:00"
...
def ts_to_local(ts: int, user_id: int | None = None, row=None) -> datetime:
    dt = datetime.fromtimestamp(ts, BASE_TIMEZONE)
    return dt + timedelta(minutes=_clock_offset_minutes_from_setting(user_id=user_id, row=row))
...
def is_in_sleep_window(now_ts: int, start_str: str | None = None, end_str: str | None = None, *, user_id: int | None = None, row=None) -> bool:
    start_str = normalize_hhmm(start_str or SLEEP_START)
    end_str = normalize_hhmm(end_str or SLEEP_END)
    start_hour, start_minute = parse_hhmm(start_str)
    end_hour, end_minute = parse_hhmm(end_str)
...
def bar(value: int, max_value: int = 4, full: str = "♥", empty: str = "♡") -> str:
    value = max(0, min(max_value, value))
    return full * value + empty * (max_value - value)


def age_days(row) -> int:
    return max(0, (int(time.time()) - row["birth_at"]) // (24 * 60 * 60))


def current_time_label(row=None, user_id: int | None = None) -> str:
    return local_now(user_id=user_id, row=row).strftime("%H:%M")


def call_stage_label(row) -> str:
...
def sound_label(row) -> str:
    return "ON" if row["sound_enabled"] else "OFF"
...
def sleep_window_label(row) -> str:
    setting = database.fetch_user_settings(int(row["user_id"])) if row and row.get("user_id") else None
    start = setting["sleep_start"] if setting else SLEEP_START
    end = setting["sleep_end"] if setting else SLEEP_END
    return f"{start}〜{end}"


def maybe_start_praise_event(user_id: int, row, now: int) -> None:
...
def maybe_start_good_behavior_event(user_id: int, row, now: int) -> None:
    if is_egg(row) or row["is_sleeping"] or row["call_flag"] or row["praise_pending"] or row.get("good_behavior_pending", 0):
        return
...
def image_key_for_pet(row, transient: str | None = None) -> str:
    ...
    if row["poop"] >= 1:
        return f"{name}_ウンチ"
    if row["is_sleeping"] or row["sleepiness"] >= 80:
        return f"{name}_眠い"
    if row["mood"] <= 1:
        return f"{name}_怒り"
    if row["mood"] >= 4:
...
def status_lines(row) -> str:
    ...
    away = "🏠 おるすばん中" if row.get("odekake_active", 0) else ""
...
def get_decay_profile(row):
    ...
    if stage == "adult":
        return {"hunger_minutes": 26, "mood_minutes": 30, "sleep_gain_minutes": 60, "poop_minutes": 52}
...
def evolve_if_needed(user_id: int, row):
    messages: list[str] = []
    now = int(time.time())
    cid = row["character_id"]
...
def random_event_if_due(user_id: int, row):
    now = int(time.time())
    if is_egg(row) or row["is_sleeping"] or row["call_flag"] or row.get("odekake_active", 0):
        return None
...
def update_sleep_state(user_id: int, row, now: int):
...
def update_over_time(user_id: int, row):
    now = int(time.time())
    if row["journeyed"] or row.get("odekake_active", 0):
    ...
    if diff < 30:
        ...
        updates = {}
        if warning:
            updates["evolution_warned"] = 1
        if wake_message:
            updates["is_sleeping"] = is_sleeping
            updates["lights_off"] = lights_off
        if updates:
    ...
    prev_call_stage = row.get("call_stage", 0)
    if call_flag:
        if not row["call_flag"]:
            call_started_at = now
            call_stage = 1
    ...
    if row["call_flag"] and call_stage > prev_call_stage and prev_call_stage > 0:
        care_miss_count += 1
    ...
    row = database.fetch_pet(user_id)
    evo_messages = evolve_if_needed(user_id, row)
    event = random_event_if_due(user_id, row)
    return database.fetch_pet(user_id), evo_messages, wake_message or warning, event


def _night_visit_increment(row, now: int) -> int:
    current_hour = ts_to_local(now, row=row).hour
    if current_hour >= 22 or current_hour < 5:
        return row["night_visit_count"] + 1
    return row["night_visit_count"]
...
def perform_action(user_id: int, row, action: str):
    ...
    transient = None
    if not row:
        return row, "データが見つからないよ。", [], transient
    if row["journeyed"]:
        return row, "🌟 この子はもう旅立ったよ。『育成開始』で新しい子を迎えてね。", [], transient
    if row.get("odekake_active", 0) and action not in ("status",):
        return row, "🏠 いまはおるすばん中。『おるすばん終了』で戻してあげてね。", [], transient
    if is_egg(row) and action != "status":
        row, messages, warning, event = update_over_time(user_id, row)
        extra = []
        if warning:
            extra.append(warning)
        if event:
            extra.append(event)
        return row, "🥚 まだ卵の状態だよ。生まれるまでお世話はできないよ。", messages + extra, transient
    if row["is_sleeping"] and action not in ("status", "sleep"):
        ...
        if event:
            extra.append(event)
        return row, "💤 ねているよ。あさまでそっとしておこう。", messages + extra, transient

    night_visit_count = _night_visit_increment(row, now)
    night_bonus = personality_bonus(row["character_id"], "night_visit") if night_visit_count != row["night_visit_count"] else {}
    if night_bonus:
        database.update_pet(user_id, night_visit_count=night_visit_count, affection=clamp(row["affection"] + night_bonus.get("affection", 0)))
        row = database.fetch_pet(user_id)
    else:
        row = dict(row)
        row["night_visit_count"] = night_visit_count

    result = ""
    if action == "feed":
        bonus = personality_bonus(row["character_id"], "feed")
        database.update_pet(user_id, hunger=clamp_meter(row["hunger"] + 2), mood=clamp_meter(row["mood"] + bonus.get("mood", 0)), total_feed_count=row["total_feed_count"] + 1, affection=clamp(row["affection"] + 2), weight=min(30, row["weight"] + 1), call_flag=0 if row["call_reason"] == "hunger" else row["call_flag"], call_reason=None if row["call_reason"] == "hunger" else row["call_reason"], night_visit_count=night_visit_count, last_access_at=now)
        result = "🍚 ごはんをたべた！"
        transient = "feed"
    elif action == "snack":
        bonus = personality_bonus(row["character_id"], "snack")
        database.update_pet(user_id, mood=clamp_meter(row["mood"] + 1 + bonus.get("mood", 0)), stress=clamp(row["stress"] + 4), total_snack_count=row["total_snack_count"] + 1, affection=clamp(row["affection"] + 1), weight=min(30, row["weight"] + 1), night_visit_count=night_visit_count, last_access_at=now)
        result = "🍰 おやつをたべてうれしそう！"
        transient = "snack"
    elif action == "play":
        bonus = personality_bonus(row["character_id"], "play")
        database.update_pet(user_id, mood=clamp_meter(row["mood"] + 2 + bonus.get("mood", 0)), stress=clamp(row["stress"] - 15), total_play_count=row["total_play_count"] + 1, affection=clamp(row["affection"] + 3 + bonus.get("affection", 0)), call_flag=0 if row["call_reason"] == "mood" else row["call_flag"], call_reason=None if row["call_reason"] == "mood" else row["call_reason"], night_visit_count=night_visit_count, last_access_at=now)
        result = "🎵 たのしくあそんだ！"
    elif action == "sleep":
        if row["sleepiness"] < 65:
            ...
            database.update_pet(user_id, is_sleeping=1, lights_off=1, sleepiness=clamp(row["sleepiness"] - 20 + bonus.get("sleepiness", 0)), total_sleep_count=row["total_sleep_count"] + 1, call_flag=0 if row["call_reason"] == "sleepy" else row["call_flag"], call_reason=None if row["call_reason"] == "sleepy" else row["call_reason"], night_visit_count=night_visit_count, last_access_at=now)
            result = "💡 でんきをけしたよ。ぐっすりねている…"
    elif action == "status":
        database.update_pet(user_id, total_status_count=row["total_status_count"] + 1, affection=clamp(row["affection"] + 1), night_visit_count=night_visit_count, last_access_at=now)
        ...
        if row["is_whim_call"]:
            ...
            database.update_pet(user_id, discipline=row["discipline"] + 1 + bonus.get("discipline", 0), stress=clamp(row["stress"] - 5), is_whim_call=0, call_flag=0, call_reason=None, total_discipline_count=row["total_discipline_count"] + 1, night_visit_count=night_visit_count, last_access_at=now)
            result = "📏 しかった！ わがままサインがきえた。"
        else:
            database.update_pet(user_id, affection=clamp(row["affection"] - 1), stress=clamp(row["stress"] + 2), night_visit_count=night_visit_count, last_access_at=now)
            result = "📏 いまは しつけるタイミングじゃないみたい。"
    elif action == "praise":
        if row["praise_pending"] or row.get("good_behavior_pending", 0):
            database.update_pet(user_id, praise_pending=0, praise_due_at=0, good_behavior_pending=0, good_behavior_due_at=0, affection=clamp(row["affection"] + 6), mood=clamp_meter(row["mood"] + 1), stress=clamp(row["stress"] - 6), total_praise_count=row.get("total_praise_count", 0) + 1, night_visit_count=night_visit_count, last_access_at=now)
            result = "✨ ほめた！ うれしそう！"
        else:
            database.update_pet(user_id, affection=clamp(row["affection"] - 1), stress=clamp(row["stress"] + 2), night_visit_count=night_visit_count, last_access_at=now)
        ...
        database.update_pet(user_id, poop=0, stress=clamp(row["stress"] - 12 + bonus.get("stress", 0)), total_clean_count=row["total_clean_count"] + 1, call_flag=0 if row["call_reason"] == "poop" else row["call_flag"], call_reason=None if row["call_reason"] == "poop" else row["call_reason"], night_visit_count=night_visit_count, last_access_at=now)
        result = "🧹 うんちをきれいにした！"
    elif action == "medicine":
        if row["is_sick"]:
            cured = 1 if random.randint(1, 100) <= 85 else 0
            database.update_pet(user_id, is_sick=0 if cured else 1, stress=clamp(row["stress"] - 8), total_medicine_count=row["total_medicine_count"] + 1, call_flag=0 if cured and row["call_reason"] == "sick" else row["call_flag"], call_reason=None if cured and row["call_reason"] == "sick" else row["call_reason"], night_visit_count=night_visit_count, last_access_at=now)
    ...
    return row, result, messages, transient


def start_odekake(user_id: int, row):
    if not row:
        return row, "データが見つからないよ。"
    if row["journeyed"]:
        return row, "🌟 もう旅立ったあとだよ。"
    if is_egg(row):
...
def stop_odekake(user_id: int, row):
    ...
    good_events = ["ちゃんとまてた", "ひとりであそべた", "おだやかにすごせた", "じょうずにおるすばんできた"]
    bad_events = ["さみしくなった", "おなかがすいた", "すこしつかれた", "おちつかなかった"]
    lines: list[str] = []
    hunger = row["hunger"]
    mood = row["mood"]
    stress = row["stress"]
    ...
    row = database.fetch_pet(user_id)
    row, evo_messages, warning, event = update_over_time(user_id, row)
    extra = evo_messages[:]
    if warning:
        extra.append(warning)
    if event:
...
def start_minigame(user_id: int, row, game_key: str):
    if row["journeyed"]:
        return None, "🌟 もう旅立ったあとだよ。"
    if is_egg(row):
        return None, "🥚 まだ卵だから、ミニゲームは生まれてからだよ。"
    if row["is_sleeping"]:
        return None, "💤 ねているから、あそべないよ。"
    if row.get("odekake_active", 0):
        return None, "🏠 おるすばん中はミニゲームできないよ。"
    if not minigame_available(row):
        remain = max(0, MINIGAME_COOLDOWN_SECONDS - (int(time.time()) - row["last_minigame_at"]))
...
def set_display_clock_to_hhmm(user_id: int, target_hhmm: str) -> str:
    target_hhmm = normalize_hhmm(target_hhmm)
    hour, minute = parse_hhmm(target_hhmm)
    base_now = datetime.now(BASE_TIMEZONE)
    target_total = hour * 60 + minute
    current_total = base_now.hour * 60 + base_now.minute
...
def adjust_display_clock(user_id: int, delta_minutes: int) -> str:
    minutes = database.adjust_clock_offset_minutes(user_id, delta_minutes)
    sign = "+" if minutes >= 0 else "-"
    abs_minutes = abs(minutes)
    return f"{sign}{abs_minutes // 60:02d}:{abs_minutes % 60:02d}"
...
def set_sleep_window(user_id: int, start: str, end: str) -> tuple[str, str]:
    start = normalize_hhmm(start)
    end = normalize_hhmm(end)
    database.set_sleep_setting(user_id, start, end)
    return start, end
...
def call_message_text(user_mention: str, row):
    ...
    if not body:
        return None
    mode = row.get("notification_mode", "tamagotchi")
    mention = "" if mode == "mute" else user_mention
    beep = "📟 ピーピー！ " if row["sound_enabled"] and mode == "tamagotchi" else ""
    ...
    if stage <= 1:
        ...
        head = "●●● 注意アイコン点灯中"
        lead = "かなりこまってるよ！"
    parts = [head, f"{beep}{lead}"]
    if mention:
        parts.append(mention)
    parts.extend([call_reason_title(reason), body, f"👉 おすすめ：{recommended_action_label(reason)}"])
    return "\n".join(parts)


def remind_interval_seconds(row) -> int:
    stage = row.get("call_stage", 1)
    if stage <= 1:
        return CALL_STAGE_INTERVALS[1]
    if stage == 2:
        return CALL_STAGE_INTERVALS[2]
    return CALL_STAGE_INTERVALS[3]
