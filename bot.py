...
async def build_embed(user_id: int, row, transient=None):
...
async def upsert_system_log(thread: discord.Thread, user_id: int, text: str):
    ...
    if not row:
        return
    message_id = row.get("system_message_id")
    body = f"【システムログ】\n{text}"
    if message_id:
        try:
...
async def upsert_alert_log(thread: discord.Thread, user_id: int, text: str):
    ...
    if not row:
        return
    message_id = row.get("alert_message_id")
    body = f"【呼出しログ】\n{text}"
    if message_id:
        try:
            msg = await thread.fetch_message(int(message_id))
            ...
            pass
    msg = await thread.send(body)
    database.update_pet(user_id, alert_message_id=str(msg.id))
...
def remind_due(row, now: int):
    if not row.get("call_flag"):
        return False
    last = row.get("last_call_notified_at") or 0
    if last == 0:
        return True
...
async def refresh_panel_for_user(user_id: int, prefix: str = "", transient=None):
    row = database.fetch_pet(user_id)
    if not row or not row.get("panel_message_id"):
        return
    thread = None
    ...
    if row.get("call_flag") and ((not prev_call) or (prev_reason != row.get("call_reason")) or remind_due(row, now)):
    ...
    if row.get("praise_pending") and not row.get("call_flag"):
        await upsert_alert_log(thread, user_id, f"✨ ほめてサイン\n<@{user_id}>\n👉 おすすめ：ほめる")
    elif (not row.get("call_flag")) and prev_call:
...
class MainPanelView(discord.ui.View):
    ...
    async def start(self, interaction, button):
        ...
        try:
            ...
            embed = await build_embed(user.id, row)
    ...
    async def continue_btn(self, interaction, button):
        ...
        if not row or row.get("journeyed"):
        ...
        try:
            ...
            if row.get("panel_message_id"):
                ...
                if pid is not None:
                    try:
                        await thread.fetch_message(pid)
                        panel_ok = True
                    except Exception:
                        panel_ok = False
            if not panel_ok:
...
class PetView(discord.ui.View):
    ...
    async def settings(self, interaction, button):
        ...
        setting = database.fetch_sleep_setting(self.owner_id)
        offset = setting.get("clock_offset_minutes", 0) if setting else 0
        text = "\n".join([
            f"通知モード: {game_logic.notification_mode_label(row['notification_mode'])}",
            f"音: {sound}",
            f"時計補正: {offset:+d}分",
            f"ねる時間: {setting['sleep_start']} ～ {setting['sleep_end']}" if setting else "ねる時間: 22:00 ～ 07:00",
        ])
        await send_temp_interaction_message(interaction, text, view=SettingsView(self.owner_id), seconds=30)
    ...
    @discord.ui.button(label="おるすばん終了", style=discord.ButtonStyle.blurple, row=4, custom_id="pet:odekake_stop")
    async def odekake_stop(self, interaction, button):
        if not await self._owner_check(interaction):
            return
        row = database.fetch_pet(self.owner_id)
        row, result, msgs = game_logic.stop_odekake(self.owner_id, row)
        await interaction.response.defer()
        await refresh_panel_for_user(self.owner_id, prefix=result)
        thread = interaction.channel
        if isinstance(thread, discord.Thread) and msgs:
            await upsert_system_log(thread, self.owner_id, "\n".join(msgs))
...
class SettingsView(discord.ui.View):
    ...
    @discord.ui.button(label="時計+1時間", style=discord.ButtonStyle.blurple, row=3)
    async def clock_plus(self, interaction, button):
        if not await self._check(interaction):
            return
        setting = database.fetch_sleep_setting(self.owner_id)
        current = int(setting.get("clock_offset_minutes", 0))
        database.set_clock_offset_minutes(self.owner_id, current + 60)
        await send_temp_interaction_message(interaction, "時計を+1時間したよ。")
    ...
    @discord.ui.button(label="時計をJSTに戻す", style=discord.ButtonStyle.gray, row=3)
    async def clock_reset(self, interaction, button):
        if not await self._check(interaction):
            return
        database.set_clock_offset_minutes(self.owner_id, 0)
        await send_temp_interaction_message(interaction, "時計を日本時間に戻したよ。")

    @discord.ui.button(label="ねる時間22-07", style=discord.ButtonStyle.gray, row=4)
    async def sleep_default(self, interaction, button):
        if not await self._check(interaction):
            return
        database.set_sleep_setting(self.owner_id, "22:00", "07:00")
        await send_temp_interaction_message(interaction, "ねる時間を 22:00 ～ 07:00 にしたよ。")

    @discord.ui.button(label="データ整理", style=discord.ButtonStyle.red, row=4)
    async def reset_menu(self, interaction, button):
        if not await self._check(interaction):
            return
        await send_temp_interaction_message(interaction, "どこまで消すか選んでね。", view=ResetDataView(self.owner_id), seconds=20)
...
class ResetDataView(discord.ui.View):
    ...
    async def reset_pet(self, interaction, button):
        if not await self._check(interaction):
            return
        database.delete_pet(self.owner_id)
        await send_temp_interaction_message(interaction, "今の育成データを消したよ。『育成開始』からやり直してね。", seconds=12)

    @discord.ui.button(label="図鑑も全部消す", style=discord.ButtonStyle.red)
    async def reset_all(self, interaction, button):
        if not await self._check(interaction):
            return
        database.reset_user_all(self.owner_id)
        await send_temp_interaction_message(interaction, "育成データと図鑑を全部消したよ。『育成開始』からやり直してね。", seconds=12)
