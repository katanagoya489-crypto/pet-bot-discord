...
def render_settings_text(owner_id: int, row) -> str:
    ...
    return (
        ...
        f"音: {sound}\n"
        f"表示時間: {game_logic.current_time_label(user_id=owner_id)}\n"
        f"時計補正: {game_logic.clock_offset_label(row)}\n"
...
async def refresh_panel_for_user(user_id: int, prefix: str = "", transient=None):
    ...
    if row["call_flag"] and ((not prev_call) or (prev_reason != row.get("call_reason")) or remind_due(row, now)):
...
class SettingsView(discord.ui.View):
    ...
    async def clock_plus(self, interaction, button):
        if not await self._owner_only(interaction): return
        game_logic.shift_clock_minutes(self.owner_id, 60)
        row = database.fetch_pet(self.owner_id)
        await send_temp_interaction_message(interaction, f"時計を1時間進めたよ。\n\n{render_settings_text(self.owner_id, row)}", seconds=25)

    @discord.ui.button(label="時計-1時間", style=discord.ButtonStyle.gray, row=3)
    async def clock_minus(self, interaction, button):
        if not await self._owner_only(interaction): return
        game_logic.shift_clock_minutes(self.owner_id, -60)
        row = database.fetch_pet(self.owner_id)
        await send_temp_interaction_message(interaction, f"時計を1時間戻したよ。\n\n{render_settings_text(self.owner_id, row)}", seconds=25)

    @discord.ui.button(label="時計をJSTに戻す", style=discord.ButtonStyle.gray, row=4)
    async def clock_reset(self, interaction, button):
        if not await self._owner_only(interaction): return
        game_logic.reset_clock_to_jst(self.owner_id)
        row = database.fetch_pet(self.owner_id)
        await send_temp_interaction_message(interaction, f"時計を日本時間に戻したよ。\n\n{render_settings_text(self.owner_id, row)}", seconds=25)

    @discord.ui.button(label="ねる時間を設定", style=discord.ButtonStyle.blurple, row=4)
    async def sleep_window(self, interaction, button):
        if not await self._owner_only(interaction): return
        await interaction.response.send_modal(SleepWindowModal(self.owner_id))

    @discord.ui.button(label="今の育成だけ消す", style=discord.ButtonStyle.red, row=5)
    async def reset_current(self, interaction, button):
        if not await self._owner_only(interaction): return
        database.delete_pet(self.owner_id)
        await send_temp_interaction_message(interaction, "今の育成データを消したよ。『育成開始』からやりなおしてね。", seconds=20)

    @discord.ui.button(label="図鑑も全部消す", style=discord.ButtonStyle.red, row=5)
    async def reset_all(self, interaction, button):
        if not await self._owner_only(interaction): return
        database.delete_all_user_data(self.owner_id)
        await send_temp_interaction_message(interaction, "育成データと図鑑を全部消したよ。『育成開始』からやりなおしてね。", seconds=20)
...
async def image_keys(ctx):
    ...
    await ctx.send("\n".join(game_logic.image_keys_for_debug(row)))
